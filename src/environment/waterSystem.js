...
void main() {
    ...
    // Normales analíticas con paso suave para evitar bandas poligonales
    float d = 0.5;
...
void main() {
    ...
    // Muestreo del mapa de normales en coordenadas continuas
    // Para ríos, se combina una escala longitudinal suave para flujo continuo sin quiebres de quad
    float t = uTime * uFlowSpeed * 0.05;
    vec2 uvScale = uIsRiver > 0.5 
        ? vec2(vUv.x * 2.0, vWorldPosition.x * 0.05 + vWorldPosition.z * 0.05 + vUv.y * 3.0) 
        : vWorldPosition.xz * 0.15;

    vec2 uv1 = uvScale + vec2(t * 0.8, t * 0.5);
    vec2 uv2 = uvScale * 1.5 - vec2(t * 0.6, -t * 0.7);
    vec2 uv3 = uvScale * 2.8 + vec2(-t * 0.4, t * 0.9);
    ...
    vec3 normal = normalize(mix(vNormal, vec3(blendNormal.x, 1.0, blendNormal.y), 0.35));

    // Vector de reflexión de la vista respecto a la normal del agua
    vec3 reflectDir = reflect(-viewDir, normal);

    // Efecto Fresnel físico con suelo mínimo del 12% para evitar bandas poligonales oscuras en vistas cenitales
    float NdotV = max(dot(normal, viewDir), 0.0);
    float fresnel = 0.12 + (1.0 - 0.12) * pow(1.0 - NdotV, 3.5);
    ...
    // En el afluente 1 (con uRiverFadeEnd > 0.5), transición cromática hacia el lago
    if (uIsRiver > 0.5 && uRiverFadeEnd > 0.5 && vUv.y > 0.40) {
    ...
    // Opacidad de la masa de agua (con suelo sólido de opacidad para que en cenital el agua no desaparezca)
    float alpha = mix(uOpacity * 0.92, uOpacity, fresnel * 0.4);

    // Desvanecimiento suave en el origen del afluente (SOLO si uRiverFadeStart > 0.5)
    if (uIsRiver > 0.5 && uRiverFadeStart > 0.5 && vUv.y < 0.08) {
        float riverStartFade = smoothstep(0.0, 0.08, vUv.y);
    ...
    if (uIsRiver > 0.5 && uRiverFadeEnd > 0.5 && vUv.y > 0.90) {
